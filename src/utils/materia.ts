...
import { rootPath, snippetMaterialsPath, getPrivateSnippetMaterialsPath } from './vscodeEnv'
...
export const getLocalMaterials = (
  ...
  try {
    materials = fs.readdirSync(materialsFullPath).map(s => {
      ...
      try {
        commandPrompt = getFileContent(path.join(fullPath, 'config', 'commandPrompt.ejs'), true)
      } catch {}
      try {
        viewPrompt = getFileContent(path.join(fullPath, 'config', 'viewPrompt.ejs'), true)
      } catch {}
      if (!preview.img) {
        preview.img = 'https://gitee.com/img-host/img-host/raw/master/2020/11/05/1604587962875.jpg'
      }
      if (!preview.schema) {
      ...
      if (type === 'snippets') {
        try {
          template = getFileContent(path.join(fullPath, 'src', 'template.ejs'), true)
        } catch {}
      }
